...
const AddTransactionModal: React.FC<{ onSave: (txn: any, id?: string, date?: number) => void; onClose: () => void; initialData?: Transaction; language: 'ta' | 'en'; t: any; }> = ({ onSave, onClose, initialData, t }) => {
...
const App: React.FC = () => {
  const [activeTab, setActiveTab] = useState<'dashboard' | 'stock' | 'accounts' | 'profile' | 'settings'>('dashboard');
    ...
    if (user.uid && isOnline && isSupabaseConfigured) {
      ...
      try {
        ...
        if (sData) {
          ...
          setStocks(freshS);
          try { localStorage.setItem(`viyabaari_stocks_${emailKey}`, JSON.stringify(freshS)); } catch(e) { console.error(e); }
        }
        ...
        if (tData) {
          ...
          try { localStorage.setItem(`viyabaari_txns_${emailKey}`, JSON.stringify(freshT)); } catch(e) { console.error(e); }
        }
    ...
    try {
        ...
        const processedVariants = await Promise.all(variantsToProcess.map(async (v: StockVariant) => {
            ...
            if (finalImageUrl && finalImageUrl.startsWith('data:image')) {
                // Only attempt upload if online and configured
                if (isOnline && isSupabaseConfigured) {
                    try {
                        ...
                        if (uploadError) {
                            console.warn("Image upload failed, keeping local base64", uploadError);
                            // Keep finalImageUrl as base64
                        } else {
                ...
                // If offline/not configured, keep finalImageUrl as base64
            }
  ...
  const handleDeleteStock = async (id: string) => {
    ...
    try {
        setStocks(prev => {
            ...
            try { localStorage.setItem(`viyabaari_stocks_${emailKey}`, JSON.stringify(updated)); } catch(e) { console.error(e); }
            return updated;
        });
  ...
  const handleClearTransactions = async () => {
    ...
    const emailKey = getEmailKey(user.email);
    setTransactions([]);
    try { localStorage.setItem(`viyabaari_txns_${emailKey}`, '[]'); } catch(e) { console.error(e); }
    if (user.uid && isOnline && isSupabaseConfigured) {
    ...
    <div className="max-w-md mx-auto min-h-screen bg-slate-50 flex flex-col shadow-xl">
      ...
      <main className="flex-1 overflow-y-auto pb-24">
        ...
        {activeTab === 'profile' && <Profile user={user} updateUser={setUser} stocks={stocks} transactions={transactions} onLogout={async () => { 
            ...
            setUser(null); 
        }} onRestore={() => {}} language={language} onLanguageChange={(l) => { setLanguage(l); localStorage.setItem('viyabaari_lang', l); }} onClearTransactions={handleClearTransactions} onResetApp={() => {}} onSetupServer={() => setShowDatabaseConfig(true)} />}
        {activeTab === 'settings' && <Settings language={language} onLanguageChange={(l) => { setLanguage(l); localStorage.setItem('viyabaari_lang', l); }} />}
      </main>
...
const AuthScreen: React.FC<{ onLogin: (u: User) => void; language: 'ta' | 'en'; t: any; isOnline: boolean }> = ({ onLogin, language, t }) => {
    ...
    return (
      <div className="min-h-screen bg-indigo-600 flex flex-col items-center justify-center p-6 text-white">
         ...
         <div className="bg-white w-full max-w-sm rounded-[2.5rem] p-8 text-gray-800 shadow-2xl">
                ...
                <div className="mt-4">
                    <button onClick={handleGoogleLogin} className="w-full bg-white border border-gray-200 text-gray-700 p-4 rounded-2xl font-bold shadow-sm hover:bg-gray-50 transition flex items-center justify-center gap-2">
